...
const serial = async (
    ...
    arduino.pipe(new serialport.ReadlineParser({ delimiter: '\r\n' })).on('data', async (data) => {
        //console.log(data);
        const luminosidade = parseFloat(data[0]);
        ...
        if (HABILITAR_OPERACAO_INSERIR) {
            if (AMBIENTE == 'producao') {
                ...
                await poolBancoDados.execute(
                    'INSERT INTO leituraSensor (luminosidade, momento, fk_aquario) VALUES (?, ?, ?, ?, ?, now(), 1)',
                    [luminosidade]
                );
                console.log("valores inseridos no banco: ",luminosidade)
